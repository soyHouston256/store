...
import type { CartLogoPosition, LogoPosition } from '@/types/ProductType'
...
const LogoImage = styled.img<LogoImageProps>`
    ...
    ${props => {
        if ((props.$logoPosition === 'back' || props.$logoPosition === 'front-back') && props.$isBack) {
            return `
                width: 30%;
                left: 50%;
                top: 39%;
        ...
        if (props.$logoPosition === 'front-back' && !props.$isBack) {
            return `
                width: 17%;
                left: 39%;
                top: 35.5%;
...
function TShirt({color, image, logoPosition, isFlipped}: {color?: string, image: string, logoPosition?: CartLogoPosition, isFlipped?: boolean}) {
